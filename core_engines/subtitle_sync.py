...
def parse_and_save_real_srt(raw_srt_text, output_file, use_fade=False):
    ...
    final_parsed, prev_end, full_speech = [], 0.0, []
...
def generate_whisper_sync_srt(audio_file_path, story_text, groq_api_key, sub_short=False):
    """Groq Whisper ဖြင့် Word-Level Sync ချိန်ညှိပေးသော Main Function (Cloud အပေါ်တွင် အကောင်းဆုံး)"""
    try:
        ...
        elif hasattr(transcription, 'words') and transcription.words: whisper_words = transcription.words
        ...
        if total_sw > 0 and total_ww >= 3:
            ...
            for i in range(0, total_sw, chunk_size):
                ...
                start_whisper_idx = max(0, min(int((start_script_idx / total_sw) * total_ww), total_ww - 1))
                end_whisper_idx = max(0, min(int((end_script_idx / total_sw) * total_ww), total_ww - 1))
                
                w_start = whisper_words[start_whisper_idx]
                w_end = whisper_words[end_whisper_idx]
                
                start_time = w_start['start'] if isinstance(w_start, dict) else w_start.start
                end_time = w_end['end'] if isinstance(w_end, dict) else w_end.end
                    
                if end_time - start_time < min_duration: end_time = start_time + min_duration
            ...
            return False, None, "Whisper ဆီမှ Word-level timestamps အလုံအလောက် မရရှိပါ။"
