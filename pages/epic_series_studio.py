...
st.markdown("စာအုပ်ကို မှတ်ဉာဏ်သွင်းပြီးဖြစ်စေ၊ ဇာတ်ကောင်များကို ကိုယ်တိုင်ရေးသွင်း၍ဖြစ်စေ ဇာတ်လမ်းတွဲများ ဖန်တီးပါ။")
...
tab1, tab2 = st.tabs(["⚙️ Step 1: Memory Setup (မှတ်ဉာဏ် တည်ဆောက်ရန်)", "🎬 Step 2: Generate Episode (ဇာတ်လမ်းတွဲ ထုတ်ရန်)"])
    ...
    if setup_mode == "📄 PDF စာအုပ်မှ အလိုအလျောက် မှတ်သားမည်":
        ...
        if st.button("💾 မှတ်ဉာဏ်အဖြစ် သိမ်းဆည်းပါ"):
            if not series_name or not manual_chars.strip():
                st.error("⚠️ ဇာတ်လမ်းအမည်နှင့် ဇာတ်ကောင်အချက်အလက်များ ထည့်ပေးပါ။")
            else:
                manual_json = {
                    "series_title": series_name,
                    "global_narrative_style": "Third-Person Omniscient Cinematic Tone. Tell the story like a legendary historical epic.",
                    "global_art_style": "High-quality 2D digital art, masterpiece webtoon style. Strictly Ancient Myanmar/Bagan aesthetic. (Note: Mood shifts dynamically from gritty/dark in early era to golden/majestic in later era)",
                    "characters": {
                        "Manual_Characters": manual_chars
                    }
                }
                try:
                    with open(MEMORY_FILE, "w", encoding="utf-8") as jf:
                        json.dump(manual_json, jf, ensure_ascii=False, indent=4)
                    st.success("✅ မှတ်ဉာဏ်အသစ်ကို အောင်မြင်စွာ သိမ်းဆည်းပြီးပါပြီ! Step 2 သို့ သွားပါ။")
                except Exception as e:
...
with tab2:
    ...
    if memory_data:
        ...
        if script_input_mode == "🤖 AI ဖြင့် အလိုအလျောက် ရေးဆွဲမည်":
            ep_focus = st.text_area("ဒီအပိုင်းမှာ ဘာအကြောင်း အဓိက ပြောမလဲ?", placeholder="ဥပမာ - ကွမ်းဆော်ကြောင်းဖြူမင်းကြီးကို ကျည်စိုးနှင့် စုက္ကတေးတို့က လှည့်စားပြီး နန်းချကာ ရဟန်းဝတ်ခိုင်းလိုက်သည့် အဖြစ်အပျက်။")
        ...
        if st.button("🎬 အဆင့် (၂) - ဇာတ်ညွှန်းအတိုင်း ဗီဒီယို ဖန်တီးရန် (RENDER NOW)"):
            if not edited_script.strip():
                st.error("⚠️ ဇာတ်ညွှန်း ထည့်သွင်းပေးပါ။")
                st.stop()
