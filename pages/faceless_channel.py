...
# 🔴 FIX: load_key ကို Import လုပ်ထားပါသည်
...
def render_faceless_studio(api_key_input, saved_gemini, groq_key_fc):
    ...
    if st.button("🚀 CREATE FACELESS VIDEO (AUTO-MAGIC)"):
        ...
        if "Manual" in fc_script_mode:
            ...
            with st.spinner(f"⏳ [အဆင့်၁/၅] Gemini ဖြင့် {fc_duration} မိနစ်စာ ဇာတ်လမ်း ရေးသားနေပါသည်..."):
                ...
                success_ai, title, tags, script, err = generate_faceless_script(
                    api_key_input, fc_niche, fc_duration, fc_custom_topic, 
        ...
        # --- [အဆင့် ၃/၅] Visuals AI (Pollinations) ---
...
GLOBAL STYLE DNA: {current_style}. RULES: Include camera angles, lighting conditions, and extreme details. Avoid explicit/violent words. Do NOT include text or words in the prompt. MUST BE IN PURE ENGLISH.
...
Story: {fc_story_text[:500]}"""
        ...
        with st.spinner("⏳ [အဆင့်၄/၅] စာတန်းထိုးများကို ချိန်ညှိနေပါသည်..."):
            ...
            if not groq_key_to_use:
                groq_key_to_use = api_key_input # Final Fallback to Main AI Key
